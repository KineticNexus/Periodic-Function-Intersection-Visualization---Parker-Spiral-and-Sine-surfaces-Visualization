# -*- coding: utf-8 -*-
"""
Created on Fri Aug  2 22:00:36 2024

@author: halif

this plot automatically updates the slides and changes the interception height
...
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QSlider, QLabel, QLineEdit, QScrollArea
from PyQt5.QtCore import Qt
import sys
import traceback

class PeriodicIntersectionApp(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setWindowTitle('Periodic Function Intersection Visualization')
        ...
        self.params = {
            ...
            'interception_height': (0, -10, 10),  # New parameter
            'elev': (20, 0, 90),
            'azim': (45, 0, 360),
    ...
    def update_value(self, param, value):
        ...
        self.update_plot()  # Update plot in real-time
    ...
    def update_plot(self):
        try:
            ...
            # Calculate surfaces
            def spiral_surface(r, phi, theta0):
            ...
            interception_height = params['interception_height'] * params['AU']
            intersections = []
            for i in range(int(params['n_spirals'])):
                theta0 = 2 * np.pi * i / params['n_spirals']
                x_spiral, y_spiral, z_spiral = spiral_surface(r, phi, theta0)
                for i in range(x_spiral.shape[0]):
                    for j in range(x_spiral.shape[1]):
                        x, y, z = x_spiral[i,j], y_spiral[i,j], z_spiral[i,j]
                        z_sin = sinusoidal_surface(x, y)
                        if abs(z - z_sin - interception_height) < threshold:
            ...
            self.ax3d.set_title('Periodic Function Intersection')
            self.ax3d.view_init(elev=params['elev'], azim=params['azim'])

            # Set up X=0 projection plot
            self.ax_x0.set_xlabel('Y')
            self.ax_x0.set_ylabel('Z')
            ...
            self.ax_z0.set_xlabel('X')
            self.ax_z0.set_ylabel('Y')
            ...
            # Print information about potential number-theoretic patterns
            print("Investigate the distribution of points in the Z=0 plane for potential number-theoretic patterns.")
